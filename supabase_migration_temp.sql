...
-- ============================================================
-- STORAGE RLS (for Supabase Storage bucket "project-files")
-- ============================================================

-- Note: These need to be created via Supabase Dashboard → Storage → Policies
-- Or run:
...
CREATE POLICY "Project members can view files" ON storage.objects
  FOR SELECT USING (
    bucket_id = 'project-files' AND (
      EXISTS (
        SELECT 1 FROM project_members
        WHERE project_members.share_token = (SELECT split_part(name, '/', 1) FROM storage.objects WHERE storage.objects.id = objects.id)
      )
      OR
      EXISTS (
        SELECT 1 FROM project_members
        WHERE project_members.project_id::text = (SELECT split_part(name, '/', 1) FROM storage.objects WHERE storage.objects.id = objects.id)
        AND project_members.user_id = auth.uid()
      )
    )
  );
